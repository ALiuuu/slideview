 ...
 * sample code.
 */
public class IFNViewPager extends ViewGroup {
    ...
    public interface OnAdapterChangeListener {
         ...
         */
        void onAdapterChanged(@NonNull IFNViewPager viewPager,
                              @Nullable IFNAdapter oldAdapter, @Nullable IFNAdapter newAdapter);
    }

    public IFNViewPager(@NonNull Context context) {
        super(context);
        initViewPager();
    }

    public IFNViewPager(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initViewPager();
    }
     ...
     */
    public void setAdapter(@Nullable IFNAdapter adapter) {
        if (mAdapter != null) {
        ...
        final IFNAdapter oldAdapter = mAdapter;
        mAdapter = adapter;
    ...
    private void scrollToItem(int item, boolean smoothScroll, int velocity,
        ...
        if (DEBUG) {
            Log.d(TAG, "scrollToItem()  item = [" + item + "], smoothScroll = [" + smoothScroll + "], velocity = [" + velocity + "], dispatchSelected = [" + dispatchSelected + "]");
        }
        int destX = 0;
        final int width = getClientWidth();
        destX = (int) (width * Math.max(mFirstOffset, Math.min(item, mLastOffset)));
        if (smoothScroll) {
    ...
    private void loadItem() {
        while (mItems.size() < 3) {
            ...
            ii.position = -1;
            ii.object = mAdapter.instantiateItem(this);
            ii.widthFactor = 1f;
            mItems.add(ii);
        }
    }
    ...
    void dataSetChanged() {
        ...
        boolean needPopulate = mItems.size() < 1 * 2 + 1
    ...
    void populate(int newCurrentItem) {
        ...
        if (DEBUG) {
            ...
            for (int i = 0; i < mItems.size(); i++) {
                Log.i(TAG, "#" + i + ": pageoffset " + mItems.get(i).offset);
            }
        }
    ...
    private void calculatePageOffsets(int curIndex) {
        Log.e(TAG, "calculatePageOffsets: ", new Throwable());
        final int width = getClientWidth();
        final float marginOffset = width > 0 ? (float) mPageMargin / width : 0;

        int a = curIndex / 3;
        int b = curIndex % 3;
        ItemInfo itemInfo0 = mItems.get(0);
        ItemInfo itemInfo1 = mItems.get(1);
        ItemInfo itemInfo2 = mItems.get(2);
        itemInfo0.offset = 0;
        itemInfo1.offset = 1;
        itemInfo2.offset = 2;
        ...
        switch (b) {
            case 0:
                itemInfo0.offset = a * 3;
                itemInfo1.offset = a * 3 + 1;
                itemInfo2.offset = a * 3 - 1;
                break;
            case 1:
                itemInfo0.offset = a * 3;
                itemInfo1.offset = a * 3 + 1;
                itemInfo2.offset = a * 3 + 2;
                break;
            case 2:
                itemInfo0.offset = a * 3 + 3;
                itemInfo1.offset = a * 3 + 1;
                itemInfo2.offset = a * 3 + 2;
                break;
        }

        Log.d(TAG, "calculatePageOffsets()  curIndex = [" + curIndex + "]  a = [" + a + "] b = [" + b + "]");
        requestLayout();
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (ii != null) {
                    ...
                    if (lp.needsMeasure) {
                        ...
                        final int widthSpec = MeasureSpec.makeMeasureSpec((int) (childWidth), MeasureSpec.EXACTLY);
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    Log.d(TAG, "determineTargetPage() currentPage [" + currentPage + "] nextPage [" + nextPage + "]");
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText("asdfdasasdfsadfasdfasdsda", 100, 100, paint);
    ...
    class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            event.setClassName(IFNViewPager.class.getName());
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfoCompat info) {
            ...
            info.setClassName(IFNViewPager.class.getName());
