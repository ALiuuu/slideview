 ...
 * @date  2022/6/20 8:47 下午
 */

abstract class InfiniWindow {
    ...
    var externalOffset = 0f
    var isActive = false

    abstract fun onCreateView(): View
    open fun onBindData(position: Float) {}
    open fun onActive() {}
    open fun onUnActive() {}
}

class IFNViewPager2 : ViewGroup {
    ...
    private val mItems = ArrayList<InfiniWindow>()

    var windowFactory: IFNViewPager2.WindowFactory? = null
        set(value) {
            field = value
            val wasFirstLayout: Boolean = mFirstLayout
            mFirstLayout = true
            if (!wasFirstLayout) {
                populate()
            } else {
    ...
    private var mScroller = Scroller(context, sInterpolator)
    ...
    private val mFirstOffset = -Float.MAX_VALUE
    private val mLastOffset = Float.MAX_VALUE
    ...
    private var mOnPageChangeListeners: MutableList<IFNViewPager2.OnPageChangeListener>? = null
    private var mPageTransformer: IFNViewPager2.PageTransformer? = null
    ...
    private var mDrawingOrderedChildren: MutableList<View>? = null
    private val sPositionComparator = IFNViewPager2.ViewPositionComparator()
    ...
    interface OnPageChangeListener {
         ...
         * @see IFNViewPager.SCROLL_STATE_IDLE
         *
         * @see IFNViewPager.SCROLL_STATE_DRAGGING
         *
         * @see IFNViewPager.SCROLL_STATE_SETTLING
         */
...
//        ViewCompat.setAccessibilityDelegate(this, IFNViewPager2.MyAccessibilityDelegate())
    ...
    private fun scrollToItem(
        ...
        if (DEBUG) {
            Log.d(
                TAG,
                "scrollToItem()  item = [$item], smoothScroll = [$smoothScroll], velocity = [$velocity], dispatchSelected = [$dispatchSelected]"
            )
        }
        var destX = 0
        val width = getClientWidth()
        destX = (width * max(mFirstOffset, min(item.toFloat(), mLastOffset))).toInt()
        if (smoothScroll) {
    ...
    /**
     * Sets a [PageTransformer] that will be called for each attached page whenever
     ...
     */
    fun setPageTransformer(reverseDrawingOrder: Boolean, transformer: PageTransformer?, pageLayerType: Int) {
    ...
    override fun getChildDrawingOrder(childCount: Int, i: Int): Int {
        ...
        return (mDrawingOrderedChildren!![index].layoutParams as IFNViewPager2.LayoutParams).childIndex
    }
    ...
    private fun loadItem() {
        while (mItems.size < 3) {
            val ii: InfiniWindow = windowFactory!!.buildWindow()
            ii.position = -1
            ii.rootView = ii.onCreateView()
            addView(
                ii.rootView,
    ...
    fun populate(newCurrentItem: Int) {
        ...
        loadItem()
        /**
         * 计算位置，3个Window根据当前index，调整位置
         */
        calculatePageOffsets(mCurItem)
        if (DEBUG) {
            Log.i(TAG, "Current page list:")
            for (i in 0 until mItems.size) {
                Log.i(TAG, "#" + i + ": pageoffset " + mItems[i].offset)
            }
        }
        ...
        for (i in 0 until childCount) {
            ...
            val lp = child.layoutParams as IFNViewPager2.LayoutParams
            lp.childIndex = i
        }
        sortChildDrawingOrder()
        if (hasFocus()) {
            val currentFocused = findFocus()
            var ii: InfiniWindow? = currentFocused?.let { infoForAnyChild(it) }
    ...
    private fun sortChildDrawingOrder() {
        if (mDrawingOrder != DRAW_ORDER_DEFAULT) {
            ...
            mDrawingOrderedChildren?.sortWith(sPositionComparator)
        }
    }

    private fun calculatePageOffsets(curIndex: Int) {
        val width = getClientWidth()
        val marginOffset: Float = if (width > 0) mPageMargin.toFloat() / width else 0f
        val a = curIndex / 3
        val b = curIndex % 3
        val itemInfo0 = mItems[0]
        val itemInfo1 = mItems[1]
        val itemInfo2 = mItems[2]
        var check0 = false
        var check1 = false
        var check2 = false
        when (b) {
            0 -> {
                check0 = setIfNeed(itemInfo0, (a * 3).toFloat())
                check1 = setIfNeed(itemInfo1, (a * 3 + 1).toFloat())
                check2 = setIfNeed(itemInfo2, (a * 3 - 1).toFloat())
                performBindData(check0, check1, check2)
                performActive(true, false, false)
            }
            1 -> {
                check0 = setIfNeed(itemInfo0, (a * 3).toFloat())
                check1 = setIfNeed(itemInfo1, (a * 3 + 1).toFloat())
                check2 = setIfNeed(itemInfo2, (a * 3 + 2).toFloat())
                performBindData(check0, check1, check2)
                performActive(false, true, false)
            }
            2 -> {
                check0 = setIfNeed(itemInfo0, (a * 3 + 3).toFloat())
                check1 = setIfNeed(itemInfo1, (a * 3 + 1).toFloat())
                check2 = setIfNeed(itemInfo2, (a * 3 + 2).toFloat())
                performBindData(check0, check1, check2)
                performActive(false, false, true)
            }
        }
        Log.d(
            TAG,
            "calculatePageOffsets()  curIndex = [" + curIndex + "]  a = [" + a + "] b = [" + b + "] " +
    ...
    private fun setIfNeed(info: InfiniWindow, offset: Float): Boolean {
    ...
    private fun performBindData(bind0: Boolean, bind1: Boolean, bind2: Boolean) {
        mItems[0].let {
            if (bind0) {
                it.onBindData(it.offset)
            }
        }
        mItems[1].let {
            if (bind1) {
    ...
    private fun performActive(bind0: Boolean, bind1: Boolean, bind2: Boolean) {
        mItems[0].let {
            if (it.isActive != bind0) {
                it.isActive = bind0
                if (bind0) {
                    it.onActive()
                } else {
    ...
    override fun addView(child: View, index: Int, params: ViewGroup.LayoutParams) {
        ...
        val lp = params as IFNViewPager2.LayoutParams
    ...
    fun infoForChild(child: View): InfiniWindow? {
        for (i in 0 until mItems.size) {
            val ii = mItems[i]
            if (child === ii.rootView) {
                return ii
            }
        }
        return null
    ...
    fun infoForAnyChild(child: View): InfiniWindow? {
        var child = child
    ...
    fun infoForPosition(position: Int): InfiniWindow? {
        for (i in 0 until mItems.size) {
            val ii = mItems[i]
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (i in 0 until size) {
            val child = getChildAt(i)
            if (child.visibility != GONE) {
                val widthSpec = MeasureSpec.makeMeasureSpec(childWidthSize, MeasureSpec.EXACTLY)
                val heightSpec = MeasureSpec.makeMeasureSpec(childHeightSize, MeasureSpec.EXACTLY)
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        Log.v(TAG, "Positioning #mCurItem$mCurItem")
            ...
            if (child.visibility != GONE) {
                ...
                if (ii != null) {
                    ...
                    if (DEBUG) {
                        Log.v(
                            TAG, "Positioning #" + i +
                                    ":" + loff + "," + childTop + " " + child.measuredWidth
                                    + "x" + child.measuredHeight
                        )
                    }
                    child.layout(
                        loff, childTop,
                        loff + child.measuredWidth,
    ...
    protected fun onPageScrolled(position: Int, offset: Float, offsetPixels: Int) {
        ...
        mPageTransformer?.let {
            ...
            for (i in 0 until childCount) {
                ...
                it.transformPage(child, transformPos)
            }
        }
        mCalledSuper = true
    }
    ...
    private fun dispatchOnPageScrolled(position: Int, offset: Float, offsetPixels: Int) {
        mOnPageChangeListeners?.forEach {
            it.onPageScrolled(position, offset, offsetPixels)
        }
    }
                    ...
                    if (DEBUG) {
                        Log.v(
                            TAG, "Down at $mLastMotionX," + mLastMotionY
                                .toString() + " mIsBeingDragged=" + mIsBeingDragged
                                .toString() + "mIsUnableToDrag=" + mIsUnableToDrag
                        )
                    }
                }
            ...
            when (action and MotionEvent.ACTION_MASK) {
                ...
                MotionEvent.ACTION_UP ->
                    if (mIsBeingDragged) {
                        ...
                        val ii = infoForCurrentScrollPosition()
                        val marginOffset = mPageMargin.toFloat() / width
                        val currentPage = mCurItem
                        val pageOffset =
                            ((scrollX.toFloat() / width - ii!!.offset) / (ii.widthFactor + marginOffset))
                        val activePointerIndex = ev.findPointerIndex(mActivePointerId)
                        val x = ev.getX(activePointerIndex)
                        ...
                        Log.d(
                            TAG,
                            "determineTargetPage() currentPage [$currentPage] nextPage [$nextPage]"
                        )
    ...
    private fun performDrag(x: Float): Boolean {
        val needsInvalidate = false
        val deltaX = mLastMotionX - x
        mLastMotionX = x
        val oldScrollX = scrollX.toFloat()
        val scrollX = oldScrollX + deltaX
        val width = getClientWidth()
        val leftBound = width * mFirstOffset
        val rightBound = width * mLastOffset
...
//        }
     ...
     */
    private fun infoForCurrentScrollPosition(): InfiniWindow? {
        val width = getClientWidth()
        ...
        var lastWidth = 0f
        var first = true
        var lastItem: InfiniWindow? = null
        for (i in 0 until mItems.size) {
            val ii = mItems[i]
            var offset: Float
            offset = ii.offset
...
//        }
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        paint.textSize = 100f
        canvas.drawText("asdfdasasdfsadfasdfasdsda", 100f, 100f, paint)
